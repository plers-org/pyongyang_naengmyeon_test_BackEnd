...
가게 적합도와 **동일한 공식**을 사용해 두 점수의 의미를 일치시킨다.

```python
def type_match_score(user_vector, archetype):
    total = sum(abs(archetype[t] - user_vector[t]) / 4 for t in TRAITS)
    return round(1 - total / len(TRAITS), 4)     # 0.0 ~ 1.0
```

4개 유형 전부에 대해 계산 후 **내림차순 정렬**(동점 시 `TASTE_TYPES` 선언 순서로 tie-break):
...
| 유형 판정 방식 | 4축 벡터 ↔ 유형 기준 벡터 거리 | 취향 그래프와 유형이 **같은 계산 결과**에서 나와 화면 내 설명이 일관됨. `questions.md` §5-3의 "선택지 번호 카운팅"은 그래프와 따로 놀 수 있음 |
| 유형 일치도 공식 | 가게 적합도와 동일 | 두 점수가 같은 척도가 되어 비교·설명이 쉬움 |
...
2. **유형 기준 벡터 검증** — §3.1 값은 `questions.md`의 예시표에서 가져온 것으로 "확정값이 아니다"라고 명시되어 있다. 실제 응답으로 유형 분포가 한쪽에 쏠리는지 확인이 필요하다.
